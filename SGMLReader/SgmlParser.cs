...
namespace Sgml {
    ...
    public class Entity : IDisposable
    {
        ...
        public Entity(string name, Uri baseUri, TextReader stm, string proxy)
        {
            ...
            m_proxy = proxy;
            m_isHtml = (string.Compare(name, "html", true, CultureInfo.InvariantCulture) == 0);
        }
            ...
            else
            {
                ...
                switch (this.m_resolvedUri.Scheme)
                {
                    ...
                    default:
                        ...
                        if (string.Compare(actual.AbsoluteUri, this.m_resolvedUri.AbsoluteUri, true, CultureInfo.InvariantCulture) != 0)
                        {
        ...
        {
            return string.Compare(token, "CDATA", true, CultureInfo.InvariantCulture) == 0 ||
                   string.Compare(token, "SDATA", true, CultureInfo.InvariantCulture) == 0 ||
                   string.Compare(token, "PI", true, CultureInfo.InvariantCulture) == 0;      
        }
    ...
    internal class HtmlStream : TextReader
    {
        ...
        private string SniffAttribute(string name) {
            SniffWhitespace();
            string id = SniffName();
            if (string.Compare(name, id, true, CultureInfo.InvariantCulture) == 0){
                SniffWhitespace();
                if (SniffPattern("=")) {
                    SniffWhitespace();
                    return SniffLiteral();
    ...
    public class ElementDecl
    {
        ...
        public AttDef FindAttribute(string name)
        {
            ...
            AttDef a;
            m_attList.TryGetValue(name.ToUpper(CultureInfo.InvariantCulture), out a);
            return a;
        }
        ...
        {            
            ...
            if (m_exclusions != null) 
            {
                ...
                {
                    if (string.Compare(s, name, StringComparison.InvariantCultureIgnoreCase) == 0)
                        return false;
                }
            }
    ...
    public class Group
    {
        ...
        public void AddSymbol(string sym)
        {
            if (string.Compare(sym, "#PCDATA", true, CultureInfo.InvariantCulture) == 0) 
            {               
                Mixed = true;
            } 
            else 
        ...
        public bool CanContain(string name, SgmlDtd dtd)
        {
            ...
            foreach (object obj in Members) 
            {
                if (obj is String) 
                {
                    if (string.Compare((string)obj, name, StringComparison.InvariantCultureIgnoreCase) == 0)
                        return true;
                } 
            }
...
*/
    ...
    public class SgmlDtd
    {
        ...
        {
            ElementDecl el;
            m_elements.TryGetValue(name.ToUpper(CultureInfo.InvariantCulture), out el);
            return el;
        }
        ...
        void ParseMarkedSection()
        {
            ...
            if (string.Compare(name, "INCLUDE", true, CultureInfo.InvariantCulture) == 0) 
            {
                ParseIncludeSection();
            } 
            else if (string.Compare(name, "IGNORE", true, CultureInfo.InvariantCulture) == 0) 
            {
                ParseIgnoreSection();
            }
            else 
        ...
        private void ParseEntity()
        {
            ...
            else 
            {
                ...
                {
                    extid = tok;
                    if (string.Compare(extid, "PUBLIC", true, CultureInfo.InvariantCulture) == 0) 
                    {
                        ch = this.m_current.SkipWhitespace();
                        if (ch == '"' || ch == '\'') 
                        {
                    ...
                    else if (string.Compare(extid, "SYSTEM", true, CultureInfo.InvariantCulture) != 0) 
                    {
