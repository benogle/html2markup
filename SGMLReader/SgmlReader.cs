    ...
    internal class Node
    {
        ...
        public Attribute AddAttribute(string name, string value, char quotechar, bool caseInsensitive) {
            ...
            for (int i = 0, n = this.attributes.Count; i < n; i++) {
                ...
                if (string.Compare(a.Name, name, caseInsensitive, CultureInfo.InvariantCulture) == 0)
                {
                    return null;
                }
            }
        ...
        public void RemoveAttribute(string name)
        {
            for (int i = 0, n = this.attributes.Count; i < n; i++)
            {
                ...
                if (string.Compare(a.Name, name, true, CultureInfo.InvariantCulture) == 0)
                {
    ...
    public class SgmlReader : XmlReader
    {
        ...
        public override string NamespaceURI
        {
            get
            {
                ...
                if (this.m_state == State.Attr && (string.Compare(this.m_a.Name, "xmlns", true, CultureInfo.InvariantCulture) == 0))
                {
            ...
            while (!foundnode)
            {
                switch (this.m_state)
                {
                    ...
                    case State.Eof:
                        ...
                        break;
                    case State.EndTag:
                        if (string.Compare(this.m_endTag, this.m_node.Name, true, CultureInfo.InvariantCulture) == 0)
                        {
                            Pop(); // we're done!
                            this.m_state = State.Markup;
                            goto case State.Markup;
        ...
        private bool ParseTag(char ch)
        {
            ...
            else if (ch == '!')
            {
                ...
                else
                {
                    ...
                    if (string.Compare(name, "DOCTYPE", true, CultureInfo.InvariantCulture) == 0)
                    {
                        ParseDocType();
        ...
        private bool ParseStartTag(char ch)
        {
            ...
            while (ch != Entity.EOF && ch != '>')
            {
                ...
                ch = this.m_current.SkipWhitespace();
                if (string.Compare(aname, ",", true, CultureInfo.InvariantCulture) == 0 ||
                    string.Compare(aname, "=", true, CultureInfo.InvariantCulture) == 0 ||
                    string.Compare(aname, ":", true, CultureInfo.InvariantCulture) == 0 ||
                    string.Compare(aname, ";", true, CultureInfo.InvariantCulture) == 0 )
                {
                    continue;
                ...
                {
                    Attribute a = n.AddAttribute(aname, value, quote, this.m_folding == CaseFolding.None);
                    if (a == null)
                    {
        ...
        private bool ParseEndTag()
        {
            ...
            {
                Node n = (Node)this.m_stack[i];
                if (string.Compare(n.Name, name, caseInsensitive, CultureInfo.InvariantCulture) == 0)
                {
        ...
        private bool ParseConditionalBlock()
        {
            ...
            else if (string.Compare(name, "CDATA", true, CultureInfo.InvariantCulture) != 0)
            {
        ...
        private void ParseDocType()
        {
            ...
            if (ch != '>')
            {
                ...
                if (ch != '[')
                {
                    ...
                    if (string.Compare(token, "PUBLIC", true, CultureInfo.InvariantCulture) == 0)
                    {
                        ch = this.m_current.SkipWhitespace();
                        if (ch == '\"' || ch == '\'')
                        {
                    ...
                    else if (string.Compare(token, "SYSTEM", true, CultureInfo.InvariantCulture) != 0)
                    {
                ...
                if (this.m_dtd != null && string.Compare(this.m_dtd.Name, name, true, CultureInfo.InvariantCulture) != 0)
                {
            ...
            while (ch != Entity.EOF)
            {
                if (ch == '<')
                {
                    ...
                    else if (ch == '/')
                    {
                        ...
                        if (ParseEndTag() && string.Compare(this.m_endTag, this.m_node.Name, true, CultureInfo.InvariantCulture) == 0)
                        {
            ...
            if (this.m_dtd != null)
            {
                ...
                if (node.DtdType != null) { 
                    ...
                    for (i = top; i > 0; i--)
                    {
                        ...
                        if (f != null)
                        {
                            if ((i == 2) && (string.Compare(f.Name, "BODY", true, CultureInfo.InvariantCulture) == 0 )) // NOTE (steveb): never close the BODY tag too early
                                break;
                            else if (string.Compare(f.Name, this.m_dtd.Name, true, CultureInfo.InvariantCulture) == 0 )
                ...
                else if (i < top)
                {
                    ...
                    if (i == top - 1 && string.Compare(name, n.Name, true, CultureInfo.InvariantCulture) == 0 )
                    {
